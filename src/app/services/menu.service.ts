    ...
    getMenu(appMenu:string) {
        ...
        if (menuInfo) {
            ...
            this.userService.user$.subscribe((userResponse: any) => {
                ...
                if (user && userService) {
                    const role1 = user ? user.role ? user.role.filter((menu: string | string[]) => (menu.indexOf("/") === -1)) : [] : [];
                    const role2 = userService ? userService.role ? userService.role.filter((menu: string | string[]) => (menu.indexOf("/") === -1)) : [] : [];
