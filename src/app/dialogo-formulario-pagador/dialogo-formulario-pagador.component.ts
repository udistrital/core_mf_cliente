...
import { PopUpManager } from '../services/popUpManager.service';

import { DatosPagador } from '../models/datos-pagador.model';
import { AgoraService } from '../services/agora.service';
import { facturacionElectronicaMidService } from '../services/facturacionElectronicaMid.service';
import Swal from 'sweetalert2';


@Component({
  selector: 'app-dialogo-formulario-pagador',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  styleUrl: './dialogo-formulario-pagador.component.scss'
})
export class DialogoFormularioPagadorComponent implements OnInit, OnDestroy, AfterViewInit {

  revisionForm!: FormGroup;
  datosPagador!: DatosPagador;
  datosPagadorOriginal: DatosPagador | null = null;
  ...
  parametros_nomenclatura_dian: any[] = [];
  vias: any[] = [];
  interior: any[] = [];
  naturalezas = [
  ...
  constructor(
    ...
    private facturacionElectronicaService: facturacionElectronicaMidService,
...
/**
  ...
  crearForm() {
    this.revisionForm = this.builder.group({
      ...
      numeroDocumento: ['', [Validators.required, Validators.pattern(/^[0-9]{7,14}$/)]],
    ...
    if (naturalezaControl) {
      this.suscripciones.push(
        naturalezaControl.valueChanges.subscribe(value => {
    ...
    if (tipoDocumentoControl) {
      this.suscripciones.push(
        tipoDocumentoControl.valueChanges.subscribe(value => {
  ...
  consultarDatosPagador() {
    ...
    this.suscripciones.push(
      this.facturacionElectronicaService.get('tercero_pago/'+this.data.anioRecibo)
        .pipe(finalize(() => this.setLoading(false)))
        .subscribe(
          (response: any) => {
            let entries = [];
  ...
  /**
   * Actualiza los validadores del formulario según la naturaleza seleccionada
   * @param naturaleza 'N' para natural, 'J' para jurídica
   */
  private actualizarValidadoresPorNaturaleza(naturaleza: string): void {
    const controles = {
  ...
  /**
   * Actualiza la validación del dígito de verificación
   */
  private actualizarValidacionDigito(): void {
    const digitoControl = this.revisionForm.get('digito_verificacion');
   ...
   * @param state true para mostrar loading, false para ocultarlo
   */
